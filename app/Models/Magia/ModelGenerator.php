...
//cambiar de la ruta general C:\xampp\htdocs\FacturaSistema\vendor\laravel-shift\blueprint\src\Generators\ModelGenerator.php
//cambiar de la ruta relativa vendor\laravel-shift\blueprint\src\Generators\ModelGenerator.php
    ...
    protected function populateStub(string $stub, Model $model)
    {
        if (Blueprint::isLaravel8OrHigher()) {
            ...
            foreach ($model->columns() as $column) {
                if($column->name() != 'id'){
                    array_push($campos, "'".$column->name()."'");
                }
            }
            array_push($campos, "'created_at'");
            array_push($campos, "'update_at'");
            $valores_c = implode(",", $campos);
            $body = 'public $allowed = ['.$valores_c.'];'."\n";
